...
package PPresenter::Export::PostScript;
...
{   my ($export, $viewport) = @_;
    ...
    my ($pwidth, $pheight) = $export->getPaperSize;
    my ($cwidth, $cheight) = $viewport->getCanvasDimensions;
...
{   my ($export, $show, $screen) = @_;
    ...
    if(grep {$_->getDevice ne 'printer'} $show->getViewports)
...
{   my $export = shift;
    ...
    return $export->SUPER::getPaperSize($size);
}
...
{   my ($export, $show, $popup) = @_;
    ...
    print Presenter::TRACE "Exporting slides to postscript started.\n";
    ...
    print Presenter::TRACE "Exporting slides to postscript ready.\n";
